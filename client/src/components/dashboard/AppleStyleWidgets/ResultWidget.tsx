    ...
    return (
      <div className="flex flex-col items-center justify-center h-full">
        <span className={`text-2xl font-bold ${resultColor}`}>
          {result < 0 ? `-${formatCurrency(Math.abs(result))}` : formatCurrency(result)}
        </span>
    ...
    <div className="flex flex-col h-full">
      <div className={`flex flex-col mb-4 p-3 rounded-lg ${resultBg}`}>
        <div className="flex items-center">
          ...
          <div>
            <span className={`text-3xl font-bold ${resultColor}`}>{result < 0 ? `-${formatCurrency(Math.abs(result))}` : formatCurrency(result)}</span>
      ...
      <div className="space-y-3 mt-2">
        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3">
          ...
          <p className="text-lg font-semibold text-blue-600 dark:text-blue-400">
            {(data.taxStats?.ivaLiquidar || 0) < 0 ? `-${formatCurrency(Math.abs(data.taxStats?.ivaLiquidar || 0))}` : formatCurrency(data.taxStats?.ivaLiquidar || 0)}
          </p>
        </div>
        ...
        <div className="bg-amber-50 dark:bg-amber-900/20 rounded-lg p-3">
          ...
          <p className="text-lg font-semibold text-amber-600 dark:text-amber-400">
            {(data.taxStats?.irpfPagar || 0) < 0 ? `-${formatCurrency(Math.abs(data.taxStats?.irpfPagar || 0))}` : formatCurrency(data.taxStats?.irpfPagar || 0)}
          </p>
        </div>
