...
import { generateQuotePDFAsBase64 } from "@/lib/pdf";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function SendQuoteEmailDialog({ 
  ...
  // Cargar los items del presupuesto cuando se abre el diálogo
  const { data: quoteItems, isLoading: isLoadingItems } = useQuery<any[]>({
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      const pdfBase64 = await generateQuotePDFAsBase64(quote, client, quoteItems || [], company);
      
      // Enviar el PDF por email
      const response = await apiRequest("POST", `/api/quotes/${quote.id}/send-email`, {
        pdfBase64,
        recipientEmail,
      ...
      toast({
        title: "Email enviado",
        description: `El presupuesto ${quote.quoteNumber} ha sido enviado a ${recipientEmail}`,
      });
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Envía el presupuesto {quote.quoteNumber} por correo electrónico al cliente.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            <div className="col-span-4 text-sm text-muted-foreground">
              ...
              <ul className="list-disc pl-5 mt-1">
                ...
                <li>El presupuesto {quote.quoteNumber} adjunto en PDF</li>
                <li>Información de validez del presupuesto</li>
